...
# ----------------------------
# Parallel / GPU config (MVP)
# ----------------------------
# Toggle parallel execution. When False, falls back to single process.
PARALLEL = True
# Number of worker processes (set to number of CPU cores or number of GPUs).
N_WORKERS = int(os.environ.get("KERRMAP_N_WORKERS", "1"))
# Optional: comma-separated GPU ids to pin workers to (e.g., "0,1,2"). If empty, CPU is used.
GPU_IDS = [g.strip() for g in os.environ.get("KERRMAP_GPU_IDS", "").split(",") if g.strip() != ""]
...
SAVE_PREFIX = "snr_ratio_ker_2modes"  # output prefix for HDF5 and PNG
...
def _count_and_filter(chunk_arrays, thr, device_id=None):
    ...
    If `device_id` is not None, pin this worker to that CUDA device.
    Returns (pts_chunk, modes_chunk) where pts_chunk is (M,7), modes_chunk is (M,4).
    """
    # Pin CUDA device *before* FEW is imported/built in this process
    if device_id is not None:
        os.environ["CUDA_VISIBLE_DEVICES"] = str(device_id)
    # Ensure FEW is initialized in this process (imports happen inside build_few)
    _ = get_few()
...
def _chunkify(total_len, n_chunks):
    step = ceil(total_len / n_chunks)
    return [(i*step, min((i+1)*step, total_len)) for i in range(n_chunks) if i*step < total_len]

def _device_for_worker(worker_idx):
    if not GPU_IDS:
        return None
...
def random_scan_one_mode(n_samples: int, seed: int, n_skip: int):
    """
    Latin Hypercube sampling over the full parameter box, keeping tuples where
    ...
    # Generate the full block at the proper offset (single process; cheap)
    ...
    # Parallel path: split arrays into chunks and process with a pool
    idx_chunks = _chunkify(total, N_WORKERS)
    results_pts = []
    results_modes = []

    with ProcessPoolExecutor(max_workers=N_WORKERS) as ex:
        futures = []
        for w_idx, (i0, i1) in enumerate(idx_chunks):
            chunk = (l1[i0:i1], l2[i0:i1], aa[i0:i1], pp0[i0:i1], ee0[i0:i1], thh[i0:i1], phh[i0:i1])
            dev = _device_for_worker(w_idx)
            futures.append(ex.submit(_count_and_filter, chunk, THR_SNR, dev))
        for fut in as_completed(futures):
            pts_c, modes_c = fut.result()
            if pts_c.size:
                results_pts.append(pts_c)
                results_modes.append(modes_c)

    if results_pts:
        pts_out = np.concatenate(results_pts, axis=0)
        modes_out = np.concatenate(results_modes, axis=0)
    else:
        pts_out = np.empty((0,7), float)
        modes_out = np.empty((0,4), int)
...
def main():
    ...
    if PARALLEL and N_WORKERS > 1:
        print(f"[diag] parallel: {N_WORKERS} workers; GPUs={GPU_IDS if GPU_IDS else 'CPU'}")
    else:
        print("[diag] parallel: disabled (single process)")
